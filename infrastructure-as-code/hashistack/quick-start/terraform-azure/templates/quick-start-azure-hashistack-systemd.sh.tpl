...
echo "[---Begin quick-start-azure-hashistack-systemd.sh---]"
...
LOCAL_IPV4=$(ip route get 8.8.8.8 | awk '{print $NF; exit}')
...
cat <<CONFIG | sudo tee $CONSUL_CONFIG_FILE
{
  ...
  "retry_join": ["provider=azure resource_group=${name} vm_scale_set=${name} subscription_id=${azure_subscription_id} tenant_id=${azure_tenant_id} client_id=${azure_client_id} secret_access_key=${azure_client_secret}"]
}
CONFIG
